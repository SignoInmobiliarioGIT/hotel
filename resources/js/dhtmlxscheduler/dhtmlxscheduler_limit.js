Scheduler.plugin(function (e) {
    e.config.limit_start = null, e.config.limit_end = null, e.config.limit_view = !1, e.config.check_limits = !0, e.config.mark_now = !0, e.config.display_marked_timespans = !0, e._temp_limit_scope = function () {
            ...
            for (var _ in d) {
                ...
                if (o[_])
                    for (var c in o[_]) {
                        var h = o[_][c],
                            u = h[l];
                        t[u] && (s = o._add_timespan_zones(s, e._get_blocked_zones(a[c], t[u], i, n, r)))
                    }
            }
            return s = o._add_timespan_zones(s, e._get_blocked_zones(a, "global", i, n, r))
        }
        var a = null,
            i = "dhx_time_block",
            n = "default",
        ...
        e.blockTime = function (t, a, i) {
            ...
            i = i || n;
            for (var o = !0, s = new Date(a.start_date.valueOf()), d = e.date.add(s, 1, "day"), _ = e._marked_timespans; s < a.end_date; s = e.date.date_part(d), d = e.date.add(s, 1, "day")) {
                var l = +e.date.date_part(new Date(s)),
                    c = s.getDay(),
                    h = t(a, _, c, l, i);
                if (h)
                    for (var u = 0; u < h.length; u += 2) {
                        var g = e._get_zone_minutes(s),
                            v = a.end_date > d || a.end_date.getDate() != s.getDate() ? 1440 : e._get_zone_minutes(a.end_date),
                            f = h[u],
                            m = h[u + 1];
                        if (f < v && m > g && !(o = "function" == typeof r && r(a, g, v, f, m))) break
                    }
            }
            return !o
        };
        var s = e.checkLimitViolation = function (t) {
            ...
            for (var _ = !0, l = 0; l < r.length; l++) {
                var c = !0,
                    d = r[l];
                d._timed = e.isOneDayEvent(d), c = !n.limit_start || !n.limit_end || d.start_date.valueOf() >= n.limit_start.valueOf() && d.end_date.valueOf() <= n.limit_end.valueOf(), c && (c = !e.checkInMarkedTimespan(d, i, function (e, t, i, n, r) {
                    var o = !0;
                    return t <= r && t >= n && ((1440 == r || i < r) && (o = !1), e._timed && a._drag_id && "new-size" == a._drag_mode ? (e.start_date.setHours(0), e.start_date.setMinutes(r)) : o = !1), (i >= n && i < r || t < n && i > r) && (e._timed && a._drag_id && "new-size" == a._drag_mode ? (e.end_date.setHours(0), e.end_date.setMinutes(n)) : o = !1), o
                })), c || (c = a.checkEvent("onLimitViolation") ? a.callEvent("onLimitViolation", [d.id, d]) : c), _ = _ && c
            }
        ...
        e._get_blocked_zones = function (e, t, a, i, n) {
                var r = [];
                if (e && e[t])
                    for (var o = e[t], s = this._get_relevant_blocked_zones(a, i, o, n), d = 0; d < s.length; d++) r = this._add_timespan_zones(r, s[d].zones);
                return r
                ...
                return !s(a) && e.config.limit_start && e.config.limit_end && (a.start_date < e.config.limit_start && (a.start_date = new Date(e.config.limit_start)), a.start_date.valueOf() >= e.config.limit_end.valueOf() && (a.start_date = this.date.add(e.config.limit_end, -1, "day")), a.end_date < e.config.limit_start && (a.end_date = new Date(e.config.limit_start)), a.end_date.valueOf() >= e.config.limit_end.valueOf() && (a.end_date = this.date.add(e.config.limit_end, -1, "day")),
                    a.start_date.valueOf() >= a.end_date.valueOf() && (a.end_date = this.date.add(a.start_date, this.config.event_duration || this.config.time_step, "minute")), a._timed = this.isOneDayEvent(a)), !0
                ...
                if (!this._table_view) {
                    if (this._props && this._props[this._mode]) {
                        var o, s, d = this._props[this._mode],
                            _ = d.size || d.options.length;
                        d.days > 1 ? (d.size && d.options.length && (t = (d.position + t) / d.options.length * d.size), o = t, s = t + _) : (o = 0, s = o + _);
                        for (var l = [], c = o; c < s; c++) {
                            var h = c;
                            r.days = h;
                ...
                if ("month" == this._mode) return r.days = +e.date.date_part(a), e._render_marked_timespan(r, null, null)
                ...
                var d = 0,
                    _ = 1440;
                "fullday" == t.zones && (t.zones = [d, _]), t.zones && t.invert_zones && (t.zones = e.invertZones(t.zones)), t.id = e.uid(), t.css = t.css || "", t.type = t.type || n;
                var l = t.sections;
                if (l) {
                    for (var c in l)
                ...
                for (var g = 0; g < i.length; g++) {
                    var v = i[g],
                        ...
                        m = v.end_date;
                    if (f && m)
                        for (var p = e.date.date_part(new Date(f)), y = e.date.add(p, 1, "day"); p < m;) {
                            var u = e._lame_copy({}, v);
                            delete u.start_date, delete u.end_date, u.days = p.valueOf();
                            var b = f > p ? e._get_zone_minutes(f) : d,
                                x = m > y || m.getDate() != p.getDate() ? _ : e._get_zone_minutes(m);
                            u.zones = [b, x], a.push(u), p = y, y = e.date.add(y, 1, "day")
                        } else v.days instanceof Date && (v.days = e.date.date_part(v.days).valueOf()), v.zones = t.zones.slice(), a.push(v)
                }
                return a
            }, e._get_dates_by_index = function (t, a, i) {
                var n = [];
                a = e.date.date_part(new Date(a || e._min_date)), i = new Date(i || e._max_date);
                for (var r = a.getDay(), o = t - r >= 0 ? t - r : 7 - a.getDay() + t, s = e.date.add(a, o, "day"); s < i; s = e.date.add(s, 1, "week")) n.push(s);
                return n
                ...
                if (e._table_view && "month" == e._mode) {
                    ...
                    if (a) u.push(a),
                        ...
                        g = d ? [d] : e._get_dates_by_index(i);
                        for (var v = 0; v < g.length; v++) u.push(this._scales[g[v]])
                    }
                    for (var v = 0; v < u.length; v++) {
                        a = u[v], i = g[v];
                        var f = Math.floor((this._correct_shift(i, 1) - o.valueOf()) / (864e5 * this._cols.length)),
                        ...
                        if (!this._ignores[m]) {
                            var p = this.config.rtl ? this._colsS.col_length - 1 - m : m,
                                ...
                                w = this._colsS[p],
                                k = this._colsS.heights[f] + (this._colsS.height ? this.xy.month_scale_height + 2 : 2) - 1;
                            ...
                            for (var D = 0; D < c.length; D += 2) {
                                ...
                                if (N <= E) return [];
                                var S = y.cloneNode(!0);
                                S.style.left = w + Math.round(E / 1440 * x) + "px", S.style.width = Math.round((N - E) / 1440 * x) + "px", a.appendChild(S),
                                    n.push(S)
                            }
                        }
                    }
                    ...
                    for (var v = 0; v < c.length; v += 2) {
                        var E = Math.max(c[v], 60 * r.first_hour),
                            N = Math.min(c[v + 1], 60 * r.last_hour);
                        if (N <= E) {
                            if (v + 2 < c.length) continue;
                            return []
                        }
                        ...
                        var T = 24 * this.config.hour_size_px + 1,
                            C = 36e5;
                        S.style.top = Math.round((60 * E * 1e3 - this.config.first_hour * C) * this.config.hour_size_px / C) % T + "px", S.style.lineHeight = S.style.height = Math.max(Math.round(60 * (N - E) * 1e3 * this.config.hour_size_px / C) % T, 1) + "px", a.appendChild(S), n.push(S)
                    }
                }
                return n
                ...
                if (e._table_view && "month" == e._mode)
                    for (var i in this._scales) {
                        ...
                        for (var n = new Date(e._min_date), r = 0, o = t.childNodes.length; r < o; r++) {
                            var s = t.childNodes[r];
                            s.firstChild && e._getClassName(s.firstChild).indexOf("dhx_scale_hour") > -1 || (a.push.apply(a, e._on_scale_add_marker(s, n)), n = e.date.add(n, 1, "day"))
                        }
                return a
                ...
                if (t.id = n, s) {
                    for (var _ in s)
                        if (s.hasOwnProperty(_)) {
                            ...
                            var l = s[_],
                                c = i[_];
                            c[l] || (c[l] = {}), c[l][o] || (c[l][o] = {}), c[l][o][d] || (c[l][o][d] = [], e._marked_timespans_types || (e._marked_timespans_types = {}), e._marked_timespans_types[d] || (e._marked_timespans_types[d] = !0));
                            var h = c[l][o][d];
                            t._array = h, h.push(t), r[n].push(t)
                        }
                } else {
                    i[a][o] || (i[a][o] = {}), i[a][o][d] || (i[a][o][d] = []), e._marked_timespans_types || (e._marked_timespans_types = {}), e._marked_timespans_types[d] || (e._marked_timespans_types[d] = !0);
                    var h = i[a][o][d];
                    t._array = h, h.push(t), r[n].push(t)
                }
                ...
                for (var i = 0; i < a.length; i += 2)
                    for (var n = a[i], r = a[i + 1], o = i + 2 == a.length, s = 0; s < t.length; s += 2) {
                        var d = t[s],
                            _ = t[s + 1];
                        if (_ > r && d <= r || d < n && _ >= n) a[i] = Math.min(n, d),
                            a[i + 1] = Math.max(r, _), i -= 2;
                        else {
                            if (!o) continue;
                            var l = n > d ? 0 : 2;
                            a.splice(i + l, 0, d, _)
                ...
                for (var a = e.slice(), i = 0; i < a.length; i += 2)
                    for (var n = a[i], r = a[i + 1], o = 0; o < t.length; o += 2) {
                        ...
                        if (d > n && s < r) {
                            var _ = !1;
                            n >= s && r <= d && a.splice(i, 2), n < s && (a.splice(i, 2, n, s), _ = !0), r > d && a.splice(_ ? i + 2 : i, _ ? 0 : 2, d, r), i -= 2;
                            break
                        }
                    }
                return a
            ...
            e._delete_marked_timespan_by_id = function (t) {
                ...
                if (a)
                    if (void 0 !== o) a[o] && a[o][s] && (e._addMarkerTimespanConfig(t),
                        e._delete_marked_timespans_list(a[o][s], t));
                    else
                        for (var l in a)
                            if (a[l][s]) {
                                var c = e._lame_clone(t);
                                t.days = l, e._addMarkerTimespanConfig(c), e._delete_marked_timespans_list(a[l][s], t)
                            }
                ...
                for (var i = 0; i < t.length; i++) {
                    ...
                    if (r.length) n.zones = r;
                        ...
                        t.splice(i, 1), i--;
                        for (var o = e._marked_timespans_ids[n.id], s = 0; s < o.length; s++)
                            if (o[s] == n) {
                                o.splice(s, 1);
                                break
                            }
                ...
                if (arguments.length || (e._marked_timespans = {
                    ...
                    t.start_date && t.end_date || (void 0 !== t.days || t.type || (t.days = "fullweek"), t.zones || (t.zones = "fullday"));
                    var a = [];
                    ...
                    for (var n = e._prepare_timespan_options(t), r = 0; r < n.length; r++)
                        for (var o = n[r], s = 0; s < a.length; s++) {
                            ...
                            d.type = a[s],
                                e._delete_marked_timespan_by_config(d)
                        }
                }
                ...
                if (!e._table_view || "month" == e._mode) {
                    ...
                    if (this._props && this._props[r]) {
                        ...
                        if (_.days > 1) {
                            var u = 864e5,
                                g = Math.round((a - e._min_date) / u),
                                v = _.size || l.length;
                            a = e.date.add(e._min_date, Math.floor(g / v), "day"), a = e.date.date_part(a)
                    ...
                    for (var b = 0; b < s.length; b++) d.push.apply(d, e._render_marked_timespan(s[b], t, a));
                    return d
                }
